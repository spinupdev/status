...
function computeUptimePct(daily) {
...
function latestPing(pings) {
  return pings.length ? pings[pings.length - 1] : null;
}

async function buildServiceCard(service) {
  const dir = path.join(DATA_DIR, service.slug);
  const pings = await readJson(path.join(dir, "pings.json"), []);
  const daily = await readJson(path.join(dir, "daily.json"), []);
...
function buildIncidents(incidents) {
  const sorted = [...incidents].sort((a, b) => new Date(b.start) - new Date(a.start)).slice(0, 15);
  if (!sorted.length) {
  ...
  return sorted
    .map((inc) => {
      ...
      const durationMin = end ? Math.max(1, Math.round((end - start) / 60000)) : null;
      return `
    <div class="incident">
...
.cards { display: flex; flex-direction: column; gap: 0.875rem; margin-bottom: 3rem; }
.card {
  background: color-mix(in srgb, var(--foreground) 2%, transparent);
  border: 1px solid var(--border);
  border-radius: var(--radius);
...
async function main() {
  ...
  const incidents = await readJson(path.join(DATA_DIR, "incidents.json"), []);
  ...
  for (const service of services) {
    ...
    const pings = await readJson(path.join(dir, "pings.json"), []);
    const last = latestPing(pings);
    if (!last) anyPending = true;
    else if (!last.ok) anyDown = true;
    cards.push(await buildServiceCard(service));
  }
...
<meta name="description" content="Live status and uptime history for Zeish services.">
<style>${CSS}</style>
</head>
<body>
<main>
